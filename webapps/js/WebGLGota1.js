app = (function(){
	...
	var libs = ["audioUtils.js","burbuja.js","webgl-debug.js"];
	loadLibs(libs);
	...
	setup = function () {
		...
		try {
			loadFiles([netPath+'shaders/sTexVert.c', netPath+'shaders/gota.c'], function (shaderText) {
		...
		try {
			...
			loadIR(netPath+'ir/matrix-reverb5.mp3');
			reverbGain = audioCtx.createGain();
			reverbGain.gain.value=0.2;
			reverb.connect(reverbGain);
			reverbGain.connect(masterGain);
			for(var i=0;i<cantBur;i++){
				burbu[i] = new Burbuja(audioCtx);
				burbu[i].connect(panner);
				burbu[i].connect(reverb);
			}
	...
	loadImage = function () {
		...
		//image.src = "img/cloud.jpg"; // Default image 
		image.src = netPath+"img/softCloud2.jpg"; // Default image 
		//image.src = "img/sea3.jpg"; // Default image 
		//image.src = "img/noise1.png"; // Default image
		//image.src = "img/pnoiseS1.jpg"; // Default image
	...
	return {
		init : function(){
			...
			//render();
		     ...
		     update : function(){
			     if(ready){
				     osX=-1+2*mouseX/document.documentElement.clientWidth;
				     osY=-1+2*mouseY/document.documentElement.clientHeight;
				     panner.setPosition(isFinite(osX)?osX:0.0,0.0, 0.0);
				     var canvas = document.getElementById("webglcanvas"); 
				     ...
				     if (gl) {
					     ...
					     if(fCount%del==0){
						     fCount=0;
						     del = 100 + parseInt(Math.random()*100);
						     for(var i=0;i<cantBur;i++){
							     if(i==cantBur-1){
								     burbu[i].setGVal(0.02);
								     var fat = Math.random()*200+2000;
								     burbu[i].setFAtt(fat);
								     var fdel1 = Math.random()*200+1500;
								     var fdel2 = i*0.075+0.1*Math.random();
								     burbu[i].playFDel(fdel1,fdel2);
								     //console.log('FAtt:'+fat+'FDel1:'+fdel1+'Fdel2:'+fdel2);
							     }else{
								     burbu[i].setGVal(0.03/(1+i));
								     burbu[i].setFAtt(Math.random()*2000+100);
								     burbu[i].playFDel(Math.random()*500+800,i*0.075+0.1*Math.random());
							     }
						     }
						     gl.uniform1f(gl.getUniformLocation(pictureprogram, "time"), fCount);
						     fCount++;
					     }else if(fCount%del<90){						     
						     gl.uniform1f(gl.getUniformLocation(pictureprogram, "time"), fCount);
					     }
					     gl.uniform1f(gl.getUniformLocation(pictureprogram, "offSX"), osX);
					     gl.uniform1f(gl.getUniformLocation(pictureprogram, "offSY"), osY);
		     ...
		     mousePos : function(mX,mY){
			     ...
			     mouseY=mY;
			     /*if(ready){
				burbu.setBaseF1(500+500*mouseY/document.documentElement.clientHeight);
				burbu.setBaseF2(600+1000*mouseX/document.documentElement.clientWidth);
			     }*/
		     }
